...
#include "dcmotors/RaspberryPiRoverMotors.h"
...
private:
    ...
    // --- Nova função privada adicionada ---
    // Extrai a lógica de controle de RC para uma função separada
    void handle_rc_override(const mavlink_message_t& mav_msg);
...
private:
    ...
    std::unique_ptr<openhd::telemetry::rpi::GPIOControl> m_opt_gpio_control =
        nullptr;
    std::unique_ptr<openhd::telemetry::rpi::RaspberryPiRoverMotors>
        m_opt_motor_control;
